      ...
      <div className="space-y-2">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-medium text-blue-900">Multi-Location Allocation</p>
              <p className="text-xs text-blue-700 mt-0.5">
                Consumable inventories can be split across multiple locations. Inventories marked with a location badge <span className="inline-flex items-center gap-0.5 px-1 py-0.5 text-[10px] font-medium bg-blue-50 text-blue-700 rounded border border-blue-200"><MapPin className="w-2.5 h-2.5" />2</span> are allocated to multiple stores.
              </p>
            </div>
          </div>
        </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-2 py-1.5 text-center text-xs font-medium text-gray-600">Locations</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {store.inventories.map((inv) => {
                      const inventoryCount = inv.inventory_count || 1;
                      const isMultiLocation = inventoryCount > 1;
                      return (
                        <tr key={inv.inventory_id} className="hover:bg-gray-50">
                          <td className="px-2 py-1.5">
                            <div className="flex items-center gap-1">
                              ...
                              {isMultiLocation && (
                                <span className="inline-flex items-center gap-0.5 px-1 py-0.5 text-[10px] font-medium bg-blue-50 text-blue-700 rounded">
                          ...
                          <td className="px-2 py-1.5 text-center">
                            {isMultiLocation ? (
                              <span className="text-xs text-blue-600 font-medium cursor-help" title={`This inventory is allocated across ${inventoryCount} locations`}>
                                {inventoryCount} stores
                              </span>
                            ) : (
