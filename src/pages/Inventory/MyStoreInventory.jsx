// src/pages/Inventory/MyStoreInventory.jsx
// Primary inventory view for Stock Incharge - focused on their assigned store
...
const MyStoreInventory = () => {
  ...
  const [primaryStoreId, setPrimaryStoreId] = useState(null);

  // Initial load - get user's stores
  useEffect(() => {
    const initializeStore = async () => {
      try {
        ...
        // Set primary store (responsible_location or first store)
        let primaryStore = null;
        if (response.responsible_location) {
          primaryStore = response.responsible_location;
        } else if (stores.length > 0) {
          primaryStore = stores[0];
        }

        if (primaryStore) {
          setMyStore(primaryStore);
          setPrimaryStoreId(primaryStore.id);
          setSelectedStoreId(primaryStore.id);
        }
      } catch (err) {
  ...
  // Fetch inventory data when store selection or tab changes
  useEffect(() => {
    if (!loading && selectedStoreId) {
      fetchInventoryData();
    }
  }, [selectedStoreId, activeTab, loading]);
  ...
  const fetchInventoryData = async () => {
    try {
      ...
      const params = { location: selectedStoreId };
  ...
  const selectedStore = accessibleStores.find(s => s.id === selectedStoreId) || myStore;
  ...
  if (!myStore && accessibleStores.length === 0) {
    return (
    ...
    <div className="space-y-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-900">
                  {selectedStore?.name || 'Store Inventory'}
                </span>
                {selectedStore?.code && (
                  <span className="text-xs text-gray-500 font-mono">({selectedStore.code})</span>
                )}
                {selectedStoreId === primaryStoreId && (
                  <span className="px-1 py-0.5 text-[10px] font-medium bg-primary-100 text-primary-700 rounded">
                    My Store
                  </span>
                )}
              </div>
              <p className="text-[10px] text-gray-500">
                {selectedStoreId === primaryStoreId ? 'Your assigned store' : 'Other store in hierarchy'}
              </p>
            </div>
          </div>
              ...
              >
                {accessibleStores.map((store) => (
                  <option key={store.id} value={store.id}>
                    {store.name} {store.id === primaryStoreId ? '(My Store)' : ''}
                  </option>
                ))}
              </select>
            )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-2">
          ...
          <p className="text-lg font-bold text-blue-600">{fixedAssetsTotals.inTransit}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-2">
          <p className="text-xs text-gray-500">In Use</p>
          <p className="text-lg font-bold text-slate-600">{fixedAssetsTotals.inUse}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-2">
          <p className="text-xs text-gray-500">Issues</p>
          <p className="text-lg font-bold text-amber-600">
            {fixedAssetsTotals.underRepair + fixedAssetsTotals.damaged}
          </p>
        </div>
      </div>
...
// Fixed Assets Tab Component
const FixedAssetsTab = ({ items, totals, onView }) => {
  return (
    <div className="space-y-2">
      {/* Stats Cards */}
      ...
      {items.length > 0 && (
        <div className="flex items-center justify-between text-xs text-gray-600 px-1">
          <span>
            {items.length} item{items.length !== 1 ? 's' : ''} with{' '}
            {totals.total} instance{totals.total !== 1 ? 's' : ''}
          </span>
        </div>
      )}
    </div>
...
const ConsumablesTab = ({ items, totals, onView }) => {
  return (
    <div className="space-y-2">
      {/* Stats Cards */}
...
// Perishables Tab Component
const PerishablesTab = ({ items, totals, onView }) => {
  return (
    <div className="space-y-2">
      {/* Stats Cards */}
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                      ...
                      <td className="px-3 py-2 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {item.fresh_batches > 0 && (
                            <span
                              className="inline-flex items-center px-1 py-0.5 text-xs bg-green-50 text-green-700 rounded"
                              title="Fresh"
                            >
                              {item.fresh_batches}
                            </span>
...
export default MyStoreInventory;
